...
Layer::Layer(const int prev_node_num, const int node_num,
    ...
    : prev_node_num(prev_node_num), node_num(node_num),
    weight_adam(prev_node_num * node_num), bias_adam(node_num),
    c_acts(batch_capacity, node_capacity)
{
    ...
    CUDA_CHECK( cudaMemcpy(d_weights, &tmp_weights[0],
                    sizeof(float) * weight_size, cudaMemcpyHostToDevice) );
    CUDA_CHECK( cudaMemcpy(d_biases, &tmp_biases[0],
                    sizeof(float) * node_num, cudaMemcpyHostToDevice) );

    CUDA_CHECK( cudaMalloc(&d_c_bp_deltas, sizeof(float) * node_capacity) );
    CUDA_CHECK( cudaMemset(d_c_bp_deltas, 0, sizeof(float) * node_capacity) );
}

Layer::~Layer() {
    CUDA_CHECK( cudaFree(d_weights) );
    CUDA_CHECK( cudaFree(d_biases) );
    CUDA_CHECK( cudaFree(d_c_bp_deltas) );
}
...
void Layer::update_weights(const int thread_num, const float lr) {
    ...
    update_weights_knl<<<weight_block_num, thread_num>>>(
        d_weights, weight_adam.d_ts, weight_adam.d_moms,
        weight_adam.d_vels, lr, weight_size);
}
...
void Layer::update_biases(const int thread_num, const float lr) {
    ...
    update_weights_knl<<<bias_block_num, thread_num>>>(
        d_biases, bias_adam.d_ts, bias_adam.d_moms,
        bias_adam.d_vels, lr, node_num);
}
